...
// إعادة توجيه ذكية حسب نوع صفحة الدخول الحالية
if ($type === 'admin' && is_admin()) {
    redirect(BASE_URL . 'admin/index.php');
}

if ($type === 'vendor' && is_vendor()) {
    redirect(BASE_URL . 'vendor/index.php');
}

if ($type === 'driver' && is_driver()) {
    redirect(BASE_URL . 'driver/index.php');
}

if ($type === 'user' && is_logged_in()) {
...
<script>
// ======= DEBUG HELPER - احذفه بعد حل المشكلة =======
...
window.fetch = async function(...args) {
    ...
    try {
        ...
        try {
            ...
            console.error('❌ NOT valid JSON! Parse error:', e.message);
